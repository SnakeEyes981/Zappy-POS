                ...
                <div className="item-info flex flex-col basis-[100%] justify-between sm:px-3 sm:py-1.5 px-2 py-4">
                    <div className="">
                        <div className="flex justify-between">
                            <h5 className="category md:text-base text-sm font-medium text-stone-700 capitalize">{itemDetails.itemCategory}</h5>
                            <p className="price md:text-base text-sm font-bold text-stone-800">{"$" + itemDetails.itemPrice}</p>
                        </div>
                        <div className="">
                            <h5 className="name text-stone-900 text-base sm:text-sm md:text-base font-bold h-16 md:font-extrabold capitalize">{itemDetails.itemName}</h5>
                        </div>
                    </div>
                    <div className="flex justify-start">
                        <div className="flex items-center gap-x-6">
                            <button disabled={!isActive} onClick={() => {if (counter === 1) {setIsActive(false); setCounter(0);} else setCounter(counter - 1); injectData(itemDetails, counter-1, counter-1 === 0 ? true : false)}} className={`${isActive ? 'bg-stone-900' : 'bg-stone-600 cursor-no-drop'} p-1 w-6 h-6 rounded-full text-white inline-flex items-center justify-center`}><i className="text-sm fa-solid fa-minus"></i></button>
                            <span className={`${isActive ? 'text-stone-900' : ' text-stone-600'} font-black text-lg cursor-no-drop`}>{counter}</span>
                            <button disabled={!isActive} onClick={() => {setCounter(counter + 1); injectData(itemDetails, counter+1, false)}} className={`${isActive ? 'bg-stone-900' : 'bg-stone-600 cursor-no-drop'} p-1 w-6 h-6 rounded-full text-white inline-flex items-center justify-center`}><i className="text-sm fa-solid fa-plus"></i></button>
                        </div>
                    </div>
                </div>
            </div>
