export default function Kdsordercards({order, viewOrder}) {
    return (
        <div key={order._id} className="col-span-3 md:col-span-1 p-2 bg-stone-900 text-white rounded-xl shadow">
            <div className="flex gap-y-2 flex-col justify-between h-full">
                <div className="flex lg:flex-row flex-col lg:gap-x-3 lg:gap-y-0 gap-y-2">
                    ...
                    <div className="flex flex-col w-full">
                        <div className="flex justify-between">
                            ...
                            <h3 className="font-light">{"#"+order._id.slice(0,7)}</h3>
                        </div>
                        <div className="flex justify-between">
                ...
                <div className="flex flex-col gap-y-2">
                    <button onClick={() => viewOrder(order.items, order._id, order.status)} className="border-white border hover:bg-white hover:text-stone-900 transition rounded-xl py-1.5 font-bold">View Order</button>
                </div>
            </div>
        </div>
    );
